...
The concept of pure state arises from the need to write the state of a system in a different way than that used so far in this document. In particular, if one prepares a state in a statistical mixture it is impossible to write this state as $\ket{\psi}$  --- for details on the statistical mixture, find the Appendix~\ref{ann:statmix}, however, we recommend to read the Section~\ref{sec:qmt} on quantum measurement theory before. Therefore, a new formalism to describe states appeared which defines density matrices $\rho$.
