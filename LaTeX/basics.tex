...
The concept of pure state arises from the need to write the state of a system in a different way than that used so far in this document. In particular, if one prepares a state in a statistical mixture it is impossible to write this state as $\ket{\psi}$\footnote{Further details are present in the Appendix~\ref{ann:statmix}, we however recommend to read the Section~\ref{sec:qmt} on quantum measurement theory before.}. Therefore, a new formalism to describe states appeared which defines density matrices $\rho$.

A density matrix $\rho$ is an operator of the Hilbert space $\mathcal{H}$ and of its dual $\mathcal{H^*}$ which follows a set of properties:
\begin{itemize}\label{prop:rho}
