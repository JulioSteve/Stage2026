...
Before looking at the different pictures we will use throughout this work, we need to introduce the definition of the expectation value of an operator $O$. Depending on the purity of the state, it is defined as
\begin{align*}
...
\begin{equation}
    \boxed{\dert{A_H} = \frac{i}{\hbar}\left[H_{H},A_{H}\right] + \derpart{A_H}\label{eq:HSlink}},
\end{equation}
where $\derpart{A_H(t)}=U^\dagger(t,t_0)\derpart{A_S(t)}U(t,t_0)$. We observe that if the observable does not depend on time explicitly (the second term is 0 in the last equations), then the dynamics of the operator is fully described in the Heisenberg picture.
...
\end{align}
From now on, we denote the state of the total system as $\bm{\varrho}$. Knowing that $\ket{\psi(t)}=U(t,t_0)\ket{\psi}$, we can write that
\begin{equation}\label{eq:evolrhoU}
    \bm{\varrho}(t) = U(t,t_0)\bm{\varrho}(t_0)U^\dagger(t,t_0).
\end{equation}
The evolution operator $U$ is the solution of the Schrödinger equation for the total system $T$. We know from Section~\ref{Ii1} that we can write the solution of the Schrödinger equation as an exponential when dealing with a time-independent Hamiltonian. However here, the total Hamiltonian $H_T$ is obviously time-dependent (only $H_0$ is assumed constant in time). Nevertheless, a unitary solution still exists~\cite{Breuer} and is denoted by $U$ hereafter. the free system, governed by $H_0$, follows an unitary dynamics described by $U_0(t,t_0)=e^{-\frac{i}{\hbar}H_0 (t-t_0)}$. Thus, we can define the interaction unitary operator $U_I$ as
\begin{align}
...
\begin{align*}
    \expval{O(t)} &= \Tr{\bm{\varrho}(t)O(t)}\\
    &= \mathrm{Tr}\,[\underbrace{U_I(t,t_0)\bm{\varrho}(t_0)U_I^\dagger(t,t_0)}_{\tilde{\bm{\varrho}}(t)}\,\underbrace{U_0^\dagger(t,t_0)O(t)U_0(t,t_0)}_{\tilde{O}(t)}].
\end{align*}
We can thus define in the interaction picture:
\begin{align}
    \tilde{\bm{\varrho}}(t)&=U_I(t,t_0)\bm{\varrho}(t_0)U_I^\dagger(t,t_0),\label{eq:rhoIS}\\
    \tilde{O}(t) &= U_0^\dagger(t,t_0)O(t)U_0(t,t_0)\label{eq:obsIS}.
\end{align}
We can now reverse the Eq.~(\ref{eq:evolrhoU}) and then use the Eq.~(\ref{eq:Utotal}) to get
\begin{equation*}
    \bm{\varrho}(t_0) = U^\dagger(t,t_0)\bm{\varrho}(t)U(t,t_0) = U_I^\dagger(t,t_0)U^\dagger_0(t,t_0)\bm{\varrho}(t)U_0(t,t_0)U_I(t,t_0),
\end{equation*}
and, by injecting this into the Eq.~(\ref{eq:rhoIS}), we get
\begin{equation}
    \tilde{\bm{\varrho}}(t) = U^\dagger_0(t,t_0)\bm{\varrho}(t)U_0(t,t_0).
\end{equation}
...
\begin{equation}
    \boxed{\tilde{\bm{\varrho}}(t)=U_I(t,t_0)\bm{\varrho}(t_0)U_I^\dagger(t,t_0)= U^\dagger_0(t,t_0)\bm{\varrho}(t)U_0(t,t_0)}.
\end{equation}
...
\begin{equation}
    \qty(\dert{\bm{\varrho}}=-\frac{i}{\hbar}\qty[H_T,\bm{\varrho}])\Leftrightarrow\qty(\dert{\tilde{\bm{\varrho}}}=-\frac{i}{\hbar}\qty[\tilde{H}_I,\tilde{\bm{\varrho}}]),
\end{equation}
...
We used in Eq.~(\ref{eq:rhoTevol}) the following approximation: $U_I(t+\dt,t)\approx e^{-\frac{i}{\hbar}\tilde{H}_I(t)\dt}$, and we would like to explain it here. 
...
\begin{equation}
    U_I(t+\dt,t_0)=U_I(t,t_0)+\dert{U_I}(t,t_0)\dt + o(\dt).
\end{equation}
We can then inject Eq.~(\ref{eq:UI}) into this expansion to get
\begin{equation}
    U_I(t+\dt,t_0)=U_I(t,t_0)-\frac{i}{\hbar}\tilde{H}_I(t,t_0)U_I(t,t_0)\dt + o(\dt).
\end{equation}
By setting $t=t_0$ and using the condition on unitary operator $U_I(t_0,t_0)$ we obtain
\begin{equation}
    U_I(t+\dt,t)=\mathbb{I}-\frac{i}{\hbar}\tilde{H}_I(t)\dt + o(\dt).
\end{equation}
Note that technically, $\tilde{H}_I(t_0,t_0)=H_I(t_0)$ for the same reason because of $U_0$. However we are interested into using the expression of $\tilde{H}_I$ in the calculation of Section~\ref{OQS} and so, we decide to keep the expression in the interaction picture. We can observe that this expansion corresponds to the expansion of the exponential at the same order, indeed
\begin{equation}
    e^{-\frac{i}{\hbar}\tilde{H}_I(t)\dt}=\mathbb{I}-\frac{i}{\hbar}\tilde{H}_I(t)\dt + o(\dt),
\end{equation}
thus proving the expression we used:
\begin{equation}
    U_I(t+\dt,t)\approx e^{-\frac{i}{\hbar}\tilde{H}_I(t)\dt}.
\end{equation}