...
\end{equation}
The evolution operator $U$ is the solution of the Schrödinger equation for the total system $T$. We know from Section~\ref{Ii1} that we can write the solution of the Schrödinger equation when having a time-independent Hamiltonian as an exponential. However here, the total Hamiltonian $H_T$ is obviously time-dependent (only $H_0$ is assumed constant in time). Nevertheless, a unitary solution still exists in this case~\cite{Breuer}: the free system (dynamically governed by $H_0$) follows unitary dynamics described by $U_0(t,t_0)=e^{-\frac{i}{\hbar}H_0 (t-t_0)}$. Thus, we can define the interaction unitary operator $U_I$ as
\begin{align}
