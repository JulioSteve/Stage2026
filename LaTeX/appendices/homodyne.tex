...
\end{equation}
As explained, the field input labelled by $\hat{b}$ is a coherent field. Let us note it as $\ket{\beta}$, this state respects: $\hat{b}\ket{\beta}=\beta\ket{\beta}$, where $\beta=\abs{\beta}e^{i\phi}\in\mathbb{C}$. If we denote as $\ket{\psi}$ the input state of the system, then the total input state is $\ket{\psi_\text{in}}=\ket{\psi} \otimes \ket{\beta}$. We can therefore explicit the average in Eq.~(\ref{eq:D2}) as
\begin{equation}
