...
\end{equation}
where we used the exponential notation for the correlations (which are complex scalars). In particular, since $\rho_{ge}=\rho_{eg}^*$ we can write that $\rho_{eg}=\abs{\rho_{eg}}e^{i\varphi}$ and $\rho_{ge}=\abs{\rho_{eg}}e^{-i\varphi}$. We can recognize the expression of a cosinus, explicitly
\begin{equation}
...
\end{equation}
Hence in dimensionless units (and with the initial condition $t_0=0$),
\begin{equation}
    \rho_{eg}(\tau) = \rho_{eg}(0)e^{-\frac{\tau}{2}}\qty[\mathrm{cos}\qty(\Omega\tau)-i\mathrm{sin}\qty(\Omega\tau)].
\end{equation}
...
\begin{equation}
    ...
    \end{cases}.
\end{equation}
This system of equation are giving the dashed blue curves on graphs of Figure~\ref{fig:HQglobale}, while the averaged trajectories are the two solid orange curves. 
...
\end{equation}
we can analytically calculate the mean value of this operator. That is to say,
\begin{equation}
